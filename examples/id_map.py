...
for dt in dates:
    print(dt)
    df_pos = pd.DataFrame(
        data=[[dt, sedol, 1e6] for sedol in sedols],
    ...
    while first_for_date or count > 0:
        ...
        universe = schema.SecuritySearchFilterUniverse(
            type=schema.Universe.PORTFOLIO, in_=op.get_portfolio_id(portfolio)
        )
        filter.__setattr__("universe", [universe])

        security_search = oper.model(id="AXWW4-MH").security_search(
            on=dt, filter=[filter], take=200, skip=total_count
        )
        security_search.count()

        security_search.securities().__fields__(
            "id", "sedol", "model_provider_id",
        )
        print(dt, count, total_count)
        try:
            results = oper()
            data = [
                (dt, res.id, res.sedol, res.model_provider_id)
