...
def load_op_raw_return(axioma_ids, start_date, end_date):
    ...
    for id in axioma_ids:
        try:
            df_ret = op.get_stock_returns(
                "model_provider_id", [id], start_date, end_date
            )
        ...
        """,
            ...
            for n in day_counts:
                for ret_type in ["total"]:
                    ...
                    df[col_name] = (
                        (1 + df[[f"{ret_type}_return"]]).pct_change(n).shift(-n)
                    )
                for ret_type in ["idio", "sector"]:
                    col_name = f"{ret_type}_{n}d_fwd"
                    total_col_name = f"total_return"
                    df[col_name] = (
                        df.shift(-n)[f"{ret_type}_return"] - df[f"{ret_type}_return"]
                    ) / (1 + df[total_col_name])
            df = df.rename(
                columns={
            ...
            df.to_sql(
                table_name, utils.get_db_connection(), if_exists="append", index=False
            )
        except Exception as e:
            errors.append((e, id))
    return errors
